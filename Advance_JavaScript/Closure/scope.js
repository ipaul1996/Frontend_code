...
//A block is a chunk of code bounded by {}.Variables declared inside a {} block cannot be accessed from outside the block. These variable are called block-scoped variable and they have a block scope. Block-scoped variable should always be declared with let and const. If we use var then it can't be block-scoped  and can be accessed outside the block.
...
//The variables declared with var are hoisted to the top of the scope of the program and initialized with undefined. For example,
//console.log(k);
...
//Variables declared with the const maintain constant values.This means that the value of a variable declared with const remains the same within its scope. It cannot be updated or re-declared. So if we declare a variable with const, we can neither do this:
...
//This behavior is somehow different when it comes to objects declared with const. While a const object cannot be updated  with a reassignment of a new obj, the properties of this objects can be updated(Values of the properties).
//The variables declared with const are hoisted to the top of the scope of the program but are not initialized. For example,
//console.log(t);
...
//Closure means an inner function has alaways access to outer function scope i.e., it has access to all the the variables declared inside the outer function, even after the outer function has closed.
